// lib/core/services/orders_export_service.dart
...
class OrdersExportService {
  ...
  static Future<void> exportOrders(
    ...
    try {
      ...
      // titleCell.value = TextCellValue('Orders Export');
  ...
  // ⭐ نسخة للتصدير في الخلفية بدون UI
  static Future<String?> exportOrdersSilent(
    List<dynamic> orders,
  ) async {
    try {
      final excel = Excel.createExcel();
      ...
      sheet.appendRow(['TOTAL', '', '', '', '', '$grandTotal L.E']);
      sheet.appendRow([
        'Total Orders:',
        orders.length.toString(),
        '',
